    ...
    <main v-if="todosVideos.length > 0">
      ...
      <section v-if="destaque" class="relative h-[70vh] w-full cursor-pointer" @click="$router.push(`/movie/${destaque._id}`)">
        ...
        <div class="absolute bottom-1/4 left-12 max-w-xl space-y-4">
          <h2 class="text-3xl md:text-6xl font-bold uppercase leading-tight">{{ destaque.titulo }}</h2>
          ...
          <p class="text-sm md:text-lg text-gray-200 line-clamp-3">{{ destaque.descricao }}</p>
          
          <div class="flex gap-3">
            <button @click.stop="$router.push(`/movie/${destaque._id}`)" class="btn bg-red-800 px-8 text-black border-none hover:bg-white/80">
               Assistir
            </button>
      ...
      <section class="px-12 -mt-20 relative z-10 space-y-12 pb-10">
        <div>
          <h3 class="text-2xl font-semibold mb-4 ">Top 10 Mais Assistidos</h3>
          <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
           <div v-for="video in top10" :key="video._id" 
              ...
              @click="$router.push(`/movie/${video._id}`)">
                ...
                <div class="p-2 text-sm font-medium group-hover:text-red-700 group-hover:font-bold transition-colors truncate">
                  {{ video.titulo }}
                </div>
            </div>
          </div>
      ...
      <section v-for="cat in categorias" :key="cat" class="px-12 mt-5 relative z-10 space-y-12 pb-10">
        <div>
